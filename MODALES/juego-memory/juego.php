<script src="/HACKATON_ONBOARDING/MODALES/juego-memory/app.js"></script>
<link rel="stylesheet" href="/HACKATON_ONBOARDING/MODALES/juego-memory/styles.css">
<div class="modal-dialog  modal-dialog-centered">
    <div class="modal-content  w-auto" id="game">
        <div class="modal-juego d-flex w-100 align-items-center mb-3"> 
            <h1 class="fs-5 mx-auto" id="staticBackdropLabel">Nivel 1</h1>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"><i class="ai-cross"></i></button>
        </div>
        <div class="game-container">
            <div class="grid_memory" ></div>
        </div>
    </div>
</div>
    